...
namespace StructureMap.Configuration.Tokens.Properties
{
	...
	public class ChildArrayProperty : Property
	{
		...
		private void buildChildProperties(InstanceMemento[] children, PropertyDefinition definition, PluginGraphReport report)
		{
			int index = 0;
		...
		public ChildProperty[] InnerProperties
		{
			get
			{
				return (ChildProperty[]) _innerProperties.ToArray(typeof (ChildProperty));
			}
		}
		...
		public ChildProperty this[int index]
		{
			get
			{
				return (ChildProperty) _innerProperties[index];
			}
		}
