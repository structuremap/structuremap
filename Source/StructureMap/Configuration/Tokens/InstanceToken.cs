...
namespace StructureMap.Configuration.Tokens
{
	...
	public class InstanceToken : GraphObject
	{
		...
		public InstanceToken(string pluginTypeName, PluginGraphReport report, InstanceMemento memento) : this()
		{
			_memento = memento;
			_mementoString = memento.ToString();
			_pluginTypeName = pluginTypeName;
			
			this.InstanceKey = memento.InstanceKey;
			this.ConcreteKey = memento.ConcreteKey;
			try
			{
				PluginToken plugin = report.FindPlugin(pluginTypeName, this.ConcreteKey);
				if (plugin == null)
				{
					if (memento.TemplateName == string.Empty)
					{
						logInvalidPlugin(pluginTypeName);
					}
					else
					{
						buildPropertyFromTemplatedMemento(report, pluginTypeName, memento);
					}
				}
				else
				{
		...
		private void buildPropertyFromTemplatedMemento(PluginGraphReport report, string pluginTypeName, InstanceMemento memento)
		{
			_templateKey = memento.TemplateName;
			TemplateToken token = report.FindTemplate(pluginTypeName, _templateKey);
		...
		{
			string message = string.Format("Looking for Plugin '{0}' for PluginFamily {1}", this.ConcreteKey, pluginTypeName);
		...
		public IProperty this[string propertyName]
		{
			get
			{
				return (IProperty) _properties[propertyName];
			}
		}
		...
		public void Validate(IInstanceValidator validator)
		{
			try
			{
				object target = validator.CreateObject(_pluginTypeName, _memento);
		...
		{
			if (this.TemplateKey != string.Empty && this.TemplateKey != null)
			{
				return string.Format("{0} Instance {1} of Type {2} (Template {3})", this.Source.ToString(), this.InstanceKey, this.ConcreteKey, this.TemplateKey);
			}
			return string.Format("{0} Instance '{1}' of Type {2}", this.Source.ToString(), this.InstanceKey, this.ConcreteKey);
		}
	}
}
