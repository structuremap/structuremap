	...
	public class PluginFamilyAttribute : Attribute
	{
		...
		public MementoSource CreateSource(Type exportedType)
		{
			if (this.SourceType != null)
			{
				try
				{
					return (MementoSource) Activator.CreateInstance(this.SourceType);
				}
				catch (Exception ex)
				{
					throw new StructureMapException(122, ex, this.SourceType.FullName, exportedType.FullName);
				}
			}
			else
			{
		...
		{
			PluginFamilyAttribute att = Attribute.GetCustomAttribute(objectType, typeof (PluginFamilyAttribute), false) as PluginFamilyAttribute;
			return (att != null);
		}
		...
		public PluginFamily BuildPluginFamily(Type exportedType)
		{
			...
			family.InterceptionChain = builder.Build(this.Scope);
		...
		{
			return Attribute.GetCustomAttribute(exportedType, typeof (PluginFamilyAttribute), false) as PluginFamilyAttribute;
		}
