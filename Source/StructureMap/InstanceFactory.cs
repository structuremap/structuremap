...
namespace StructureMap
{
	...
	public class InstanceFactory : IInstanceFactory
	{
		...
		public InstanceFactory(PluginFamily family, bool failOnException) : this()
		{
			...
			this.processPlugins(family.Plugins);
		...
		private void determineDefaultKey(PluginFamily family, bool failOnException)
		{
			if (family.DefaultInstanceKey != null && family.DefaultInstanceKey != string.Empty)
			{
				try
				{
					this.SetDefault(family.DefaultInstanceKey);
				}
				catch (Exception)
				{
					if (failOnException)
		...
		{
			string defaultKey = PluginFamilyAttribute.GetDefaultKey(this.PluginType);
			if (defaultKey != string.Empty)
			{
				...
				{
					throw new StructureMapException(10, ex, defaultKey, this.PluginType.FullName);
				}
			}
		}
		...
		private void setPluginType(Type PluginType)
		{
			...
			_assemblyName = _pluginType.FullName.Replace(".", "") + "InstanceBuilderAssembly";
		}
		...
		{
			foreach (InstanceBuilder builder in this._instanceBuilders.Values)
			{
		...
		{
			InstanceBuilderAssembly builderAssembly = new InstanceBuilderAssembly(_assemblyName, this.PluginType);
		...
		{
			InstanceMemento memento = this.findMemento(instanceKey);
		...
		private object buildFromInstanceBuilder(InstanceMemento memento)
		{
			if (!_instanceBuilders.Contains(memento.ConcreteKey))
			{
				throw new StructureMapException(
					201, memento.ConcreteKey, memento.InstanceKey, this.PluginType.FullName);
			}
			...
			{
				if (ex is StructureMapException)
				{
					throw;
				}
				else
				{
					throw new StructureMapException(207, ex, memento.InstanceKey, this.PluginType.FullName);
				}
			}
		}
		...
		public object GetInstance()
		{
			...
			{
				throw new StructureMapException(202, this.PluginType.FullName);
			}

			return this.GetInstance(_source.DefaultMemento);
		}
		...
		{
			Array array = Array.CreateInstance(this.PluginType, Mementos.Length);
			for (int i = 0; i < Mementos.Length; i++)
			{
				object member = this.GetInstance(Mementos[i]);
		...
		public void SetDefault(string instanceKey)
		{
			...
			{
				_source.DefaultMemento = this.findMemento(instanceKey);
			}
		}
		...
		public IList GetAllInstances()
		{
			...
			{
				object instance = this.GetInstance(memento);
				list.Add(instance);
			}
