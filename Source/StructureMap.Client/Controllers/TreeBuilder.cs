...
namespace StructureMap.Client.Controllers
{
    public class TreeBuilder : IConfigurationVisitor
    {
        ...
        public GraphObjectNode BuildTree()
        {
            ...
            TreeNodeAggregator assemblyAggregator =
                new TreeNodeAggregator(typeof (AssemblyToken), ViewConstants.ASSEMBLIES, ViewConstants.ASSEMBLIES);
            assemblyAggregator.Aggregate(_topNode);

            TreeNodeAggregator familyAggregator =
                new TreeNodeAggregator(typeof (FamilyToken), ViewConstants.PLUGINFAMILIES, ViewConstants.PLUGINFAMILIES);
            GraphObjectNode familiesNode = familyAggregator.Aggregate(_topNode);

            TreeNodeAggregator[] aggregators = new TreeNodeAggregator[]
                {
                    new TreeNodeAggregator(typeof (InterceptorInstanceToken), ViewConstants.INTERCEPTORS,
                                           ViewConstants.INTERCEPTORS),
