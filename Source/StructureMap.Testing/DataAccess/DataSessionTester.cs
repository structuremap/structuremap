...
namespace StructureMap.Testing.DataAccess
{
	...
	public class DataSessionTester
	{
		...
		public void ExecuteReaderAutoCommitWithException()
		{
			...
			_autoCommitMock.ExpectAndThrow("ExecuteReader", new ApplicationException(theMessage), new IsTypeOf(typeof(IDbCommand)));
		...
		public void ExecuteDataSetHappyPath()
		{
			...
			Assert.IsTrue(object.ReferenceEquals(theDataSet, dataSet));
			_autoCommitMock.Verify();
		}
		...
		public void ExecuteScalarAutoCommitWithException()
		{
			...
			string theMessage = "bad error";
			_autoCommitMock.ExpectAndThrow("ExecuteScalar", new ApplicationException(theMessage), new IsTypeOf(typeof(IDbCommand)));
