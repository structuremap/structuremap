...
namespace StructureMap.Testing.Container.Interceptors
{
	...
	public class ThreadLocalStorageInterceptorTester
	{
		...
		{
			Thread t1 = new Thread(new ThreadStart(this.findRule1));
			Thread t2 = new Thread(new ThreadStart(this.findRule2));
			Thread t3 = new Thread(new ThreadStart(this.findRule3));
