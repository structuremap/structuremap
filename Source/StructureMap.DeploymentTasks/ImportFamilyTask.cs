...
namespace StructureMap.DeploymentTasks
{
	...
	public class ImportFamilyTask : Task
	{
		...
		{
			string message = string.Format("Importing PluginFamily '{0}' from {1} to {2}", _pluginType, _sourcePath, _targetPath);
		...
		{
			string message;
			string xpath = string.Format("//{0}[@{1}='{2}']", XmlConstants.PLUGIN_FAMILY_NODE, XmlConstants.TYPE_ATTRIBUTE, _pluginType);
