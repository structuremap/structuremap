...
namespace StructureMap.DeploymentTasks.Versioning
{
	public class DotNetAssembly
	{
		...
		{
			DotNetAssembly deployedVersion = directory.FindAssembly(this.AssemblyName);

			if (deployedVersion == null)
			{
				report.MissingAssembly(this.AssemblyName, this.Version);
			}
			else
			{
				if (this.Version != deployedVersion.Version)
				{
					report.VersionMismatchAssembly(this.AssemblyName, this.Version, deployedVersion.Version);
				}
			}
		}
	}
