...
namespace StructureMap.DataAccess.DataSetMapping
{
	public class ReaderToColumnMap : IReaderToColumnMap
	{
		...
		public void Initialize(DataTable table, IDataReader reader)
		{
			...
			if (_ordinal < 0)
			{
				throw new ArgumentOutOfRangeException("reader", "The IDataReader does not have an Ordinal for " + _readerName);
			}

			if (_column == null)
			{
				throw new ArgumentOutOfRangeException("table", "The DataTable does not have a DataColumn named " + _columnName);
			}
		}
