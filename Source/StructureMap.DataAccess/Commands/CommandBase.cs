...
namespace StructureMap.DataAccess.Commands
{
	public abstract class CommandBase : IInitializable, IReaderSource, ICommand
	{
		...
		public int Execute()
		{
			...
			{
				ex.CommandName = this.Name;
				throw;
			}
			catch (Exception ex)
			{
				throw new CommandFailureException(this.Name, _innerCommand, ex);
			}
		}
		...
		public object this[string parameterName]
		{
			get
			{
				return _parameters[parameterName].GetProperty();
			}
			set
			{
				_parameters[parameterName].SetProperty(value);
			}
		}
